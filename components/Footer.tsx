...
const ScrollToTopButton: React.FC = () => {
    ...
    return (
        <button
            type="button"
            onClick={scrollToTop}
            className={`fixed bottom-6 right-6 w-12 h-12 rounded-full text-white flex items-center justify-center shadow-lg transition-all duration-300 transform hover:scale-110 hover:brightness-110 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
            style={{ background: 'linear-gradient(135deg, #F76320, #FF8A50)', boxShadow: '0 10px 25px rgba(247, 99, 32, 0.4)' }}
